...
/**
 *
 * In lesson 02 we learn how to open an OpenGL window with a size of 800x600 and a 32-bit color depth. We set the
 * background color to gray. Then we draw a triangle and square on the screen. Using LWJGL 2.9.1.
 *
 * This code is based on NeHe's tutorials which can be found at: http://nehe.gamedev.net/
 *
 * @author Jason Sorensen
 */
public class NeheLesson02 {
    ...
    public static void main(String[] args) {
        NeheLesson02 jlesson02 = new NeheLesson02();
        jlesson02.run(args);
    }
    ...
    private void render() {
        ...
        GL11.glTranslatef(-1.5f, 0.0f, -6.0f);                                  // Move left 1.5 units And into the screen 6.0 (NEW)

        GL11.glBegin(GL11.GL_TRIANGLES);                                        // Start draw a triangle (NEW)
        GL11.glVertex3f(0.0f, 1.0f, 0.0f);                                      // Top (NEW)
        GL11.glVertex3f(-1.0f, -1.0f, 0.0f);                                    // Bottom left (NEW)
        GL11.glVertex3f(1.0f, -1.0f, 0.0f);                                     // Bottom right (NEW)
        GL11.glEnd();                                                           // Finished drawing the triangle (NEW)

        GL11.glTranslatef(3.0f, 0.0f, 0.0f);                                    // Move right 3 units (NEW)

        GL11.glBegin(GL11.GL_QUADS);                                            // Start drawing a quad (NEW)
        GL11.glVertex3f(-1.0f, 1.0f, 0.0f);                                     // Top left (NEW)
        GL11.glVertex3f(1.0f, 1.0f, 0.0f);                                      // Top tight (NEW)
        GL11.glVertex3f(1.0f, -1.0f, 0.0f);                                     // Bottom right (NEW)
        GL11.glVertex3f(-1.0f, -1.0f, 0.0f);                                    // Bottom left (NEW)
        GL11.glEnd();                                                           // Finished drawing the quad (NEW)
    }
